...
void Characterisation ( TH2D *h_smeared, TNtuple *info_s, TNtuple *info_bg ){
    ...
    TLegend *leg_T_s         = new TLegend( 0.12, 0.74, 0.28, 0.88 );
    TLegend *leg_T_bg        = new TLegend( 0.12, 0.74, 0.28, 0.88 );
    TLegend *leg_T_sbg       = new TLegend( 0.12, 0.74, 0.28, 0.88 );
    TLegend *leg_T_sbg_split = new TLegend( 0.12, 0.74, 0.28, 0.88 );
    ...
    for ( int i = 1; i <= y_bins; ++i ){
        ...
        h_Tmu_qel_s->Scale(1/norm_Tmu);
        h_Tmu_nres_s->Scale(1/norm_Tmu);
        h_Tmu_nother_s->Scale(1/norm_Tmu);
        
        h_Tmu_res_bg->Scale(1/norm_Tmu);
        h_Tmu_nres_bg->Scale(1/norm_Tmu);
        h_Tmu_other_bg->Scale(1/norm_Tmu);
        h_Tmu_nother_bg->Scale(1/norm_Tmu);
    ...
    TLegend *leg_cos_s         = new TLegend( 0.72, 0.74, 0.88, 0.88 );
    TLegend *leg_cos_bg        = new TLegend( 0.72, 0.74, 0.88, 0.88 );
    TLegend *leg_cos_sbg       = new TLegend( 0.72, 0.74, 0.88, 0.88 );
    TLegend *leg_cos_sbg_split = new TLegend( 0.72, 0.74, 0.88, 0.88 ); 
    ...
    for ( int i = 1; i <= x_bins; ++i ){
        ...
        h_cosmu_qel_s->Scale(1/norm_cosmu);
        h_cosmu_nres_s->Scale(1/norm_cosmu);
        h_cosmu_nother_s->Scale(1/norm_cosmu);
        
        h_cosmu_res_bg->Scale(1/norm_cosmu);
        h_cosmu_nres_bg->Scale(1/norm_cosmu);
        h_cosmu_other_bg->Scale(1/norm_cosmu);
        h_cosmu_nother_bg->Scale(1/norm_cosmu);
...
void Efficiency ( TH2D *h_smeared, TH2D *h_unsmeared, TH2D *h_cut, TNtuple *info_s){
    ...
    h_pur->GetZaxis()->SetRangeUser(0.7,1);
    c->Update();
    //c->SetLogz();
    c->SaveAs("purity.png");
    ...
    h_effpur->GetZaxis()->SetRangeUser(0.5,1);
    c->Update();
    //c->SetLogz();
    c->SaveAs("effpur.png");
