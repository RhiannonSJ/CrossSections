...
void Smear ( TTree   *tree, 
    ...
    for ( int i = 0; i < n_values; ++i ){        
        ...
        if ( fspl == 13 
            ...
            info_s->Fill( i, cos_pi_vect[i], T_pi_vect[i], cc, nc, nfp, nfpip, nfpim, nfpi0, nfkp, nfkm, nfk0, qel, res, mec );
        }
...
void Characterisation ( TH2D *h_smeared, TNtuple *info_s, TNtuple *info_bg ){
    ...
    TH1D *h_Tmu_qel0p_s     = new TH1D ( "h_Tmu_qel0p_s", "", x_bins, -1, 1 );
    TH1D *h_Tmu_qel1p_s     = new TH1D ( "h_Tmu_qel1p_s", "", x_bins, -1, 1 );
    TH1D *h_Tmu_qel2p_s     = new TH1D ( "h_Tmu_qel2p_s", "", x_bins, -1, 1 );
    TH1D *h_Tmu_qelmp_s     = new TH1D ( "h_Tmu_qelmp_s", "", x_bins, -1, 1 );
    TH1D *h_Tmu_res_s       = new TH1D ( "h_Tmu_res_s", "", x_bins, -1, 1 );
    TH1D *h_Tmu_mec_s       = new TH1D ( "h_Tmu_mec_s", "", x_bins, -1, 1 );
    TH1D *h_Tmu_other_s     = new TH1D ( "h_Tmu_other_s", "", x_bins, -1, 1 );
    
    signal_h.push_back(h_Tmu_qel0p_s);
    signal_h.push_back(h_Tmu_qel1p_s);
    signal_h.push_back(h_Tmu_qel2p_s);
    signal_h.push_back(h_Tmu_qelmp_s);
    signal_h.push_back(h_Tmu_res_s);
    signal_h.push_back(h_Tmu_mec_s);
    signal_h.push_back(h_Tmu_other_s);

    TH1D *h_Tmu_1cpi_bg   = new TH1D ( "h_Tmu_1cpi_bg", "", x_bins, -1, 1 );
    ...
    background_h.push_back(h_Tmu_1cpi_bg);
    ...
    leg_T_s->AddEntry( h_Tmu_qel0p_s, " CCQE, 0 protons ", "f" );
    leg_T_s->AddEntry( h_Tmu_qel1p_s, " CCQE, 1 proton ", "f" );
    leg_T_s->AddEntry( h_Tmu_qel2p_s, " CCQE, 2 protons ", "f" );
    leg_T_s->AddEntry( h_Tmu_qelmp_s, " CCQE, >2 protons ", "f" );
    leg_T_s->AddEntry( h_Tmu_res_s, " CCRES ", "f" );
    leg_T_s->AddEntry( h_Tmu_mec_s, " CCMEC ", "f" );
    leg_T_s->AddEntry( h_Tmu_other_s, " CCOther ", "f" );
   
    leg_T_bg->AddEntry( h_Tmu_1cpi_bg, " CC1#pi^{ #pm } ", "f" );
    leg_T_bg->AddEntry( h_Tmu_2cpi_bg, " CC2#pi^{ #pm } ", "f" );
    leg_T_bg->AddEntry( h_Tmu_n1cpi_bg, " NC1#pi^{ #pm } ", "f" );
    leg_T_bg->AddEntry( h_Tmu_other_bg, " CCOther ", "f" );
    leg_T_bg->AddEntry( h_Tmu_nother_bg, " NCOther ", "f" );
    
    leg_T_sbg->AddEntry( h_Tmu_sm, " Signal ", "f" );
    leg_T_sbg->AddEntry( h_Tmu_bg, " Background ", "f" );
    
    leg_T_sbg_split->AddEntry( h_Tmu_qel0p_s, " CCQE, 0 protons signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_qel1p_s, " CCQE, 1 proton signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_qel2p_s, " CCQE, 2 protons signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_qelmp_s, " CCQE, >2 protons signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_res_s, " CCRES signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_mec_s, " CCMEC signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_other_s, " CCOther signal ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_1cpi_bg, " CC1#pi background ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_2cpi_bg, " CC2#pi background ", "f" );
    leg_T_sbg_split->AddEntry( h_Tmu_n1cpi_bg, " NC1#pi^{ #pm } background ", "f" );
    ...
    for ( int i = 1; i <= y_bins; ++i ){
        ...
        for ( int k = 0; k < n_entries; ++k ){
            ...
            if ( T_s >= low_edge_T 
                ...
                h_Tmu_mec_s->Fill(cth_s,1);
            }
            else if ( T_s >= low_edge_T 
                      && T_s < up_edge_T
                      && cc_s ) {
                 
                h_Tmu_other_s->Fill(cth_s,1);
            }
        }
    ...
    TH1D *h_cosmu_qel0p_s     = new TH1D ( "h_cosmu_qel0p_s", "", y_bins, 0, 2 );
    TH1D *h_cosmu_qel1p_s     = new TH1D ( "h_cosmu_qel1p_s", "", y_bins, 0, 2 );
    TH1D *h_cosmu_qel2p_s     = new TH1D ( "h_cosmu_qel2p_s", "", y_bins, 0, 2 );
    TH1D *h_cosmu_qelmp_s     = new TH1D ( "h_cosmu_qelmp_s", "", y_bins, 0, 2 );
    TH1D *h_cosmu_res_s       = new TH1D ( "h_cosmu_res_s", "", y_bins, 0, 2 );
    TH1D *h_cosmu_mec_s       = new TH1D ( "h_cosmu_mec_s", "", y_bins, 0, 2 );
    TH1D *h_cosmu_other_s     = new TH1D ( "h_cosmu_other_s", "", y_bins, 0, 2 );
    
    signal_c_h.push_back(h_cosmu_qel0p_s);
    signal_c_h.push_back(h_cosmu_qel1p_s);
    signal_c_h.push_back(h_cosmu_qel2p_s);
    signal_c_h.push_back(h_cosmu_qelmp_s);
    signal_c_h.push_back(h_cosmu_res_s);
    signal_c_h.push_back(h_cosmu_mec_s);
    signal_c_h.push_back(h_cosmu_other_s);

    TH1D *h_cosmu_1cpi_bg   = new TH1D ( "h_cosmu_1cpi_bg", "", y_bins, 0, 2 );
    ...
    background_c_h.push_back(h_cosmu_1cpi_bg);
    ...
    leg_cos_s->AddEntry( h_cosmu_qel0p_s, " CCQE, 0 protons ", "f" );
    leg_cos_s->AddEntry( h_cosmu_qel1p_s, " CCQE, 1 proton ", "f" );
    leg_cos_s->AddEntry( h_cosmu_qel2p_s, " CCQE, 2 protons ", "f" );
    leg_cos_s->AddEntry( h_cosmu_qelmp_s, " CCQE, >2 protons ", "f" );
    leg_cos_s->AddEntry( h_cosmu_res_s, " CCRES ", "f" );
    leg_cos_s->AddEntry( h_cosmu_mec_s, " CCMEC ", "f" );
    leg_cos_s->AddEntry( h_cosmu_other_s, " CCOther ", "f" );
   
    leg_cos_bg->AddEntry( h_cosmu_1cpi_bg, " CC1#pi^{ #pm } ", "f" );
    leg_cos_bg->AddEntry( h_cosmu_2cpi_bg, " CC2#pi^{ #pm } ", "f" );
    leg_cos_bg->AddEntry( h_cosmu_n1cpi_bg, " NC1#pi^{ #pm } ", "f" );
    leg_cos_bg->AddEntry( h_cosmu_other_bg, " CCOther ", "f" );
    leg_cos_bg->AddEntry( h_cosmu_nother_bg, " NCOther ", "f" );
    
    leg_cos_sbg->AddEntry( h_cosmu_sm, " Signal ", "f" );
    leg_cos_sbg->AddEntry( h_cosmu_bg, " Background ", "f" );
    
    leg_cos_sbg_split->AddEntry( h_cosmu_qel0p_s, " CCQE, 0 protons signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_qel1p_s, " CCQE, 1 proton signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_qel2p_s, " CCQE, 2 protons signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_qelmp_s, " CCQE, >2 protons signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_res_s, " CCRES signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_mec_s, " CCMEC signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_other_s, " CCOther signal ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_1cpi_bg, " CC1#pi background ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_2cpi_bg, " CC2#pi background ", "f" );
    leg_cos_sbg_split->AddEntry( h_cosmu_n1cpi_bg, " NC1#pi^{ #pm } background ", "f" );
    ...
    for ( int i = 1; i <= x_bins; ++i ){
        ...
        for ( int k = 0; k < n_entries; ++k ){
            ...
            if ( cth_s >= low_edge_cos 
                ...
                h_cosmu_mec_s->Fill(T_s,1);
            }
            else if ( cth_s >= low_edge_cos 
                      && cth_s < up_edge_cos
                      && cc_s ) {
                 
                h_cosmu_other_s->Fill(T_s,1);
            }
        }
